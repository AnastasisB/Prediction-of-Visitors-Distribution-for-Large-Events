...
#                               X_test_ = X_test, y_test_ = y_test, n_it=iterat), estimators)
    ...
    for keys in estimators:
        ...
        bestestim[keys] = {}
        for i in range(6):
            skata[keys][chr(i+65)], bestestim[keys][chr(i+65)] = create_fit_estimator(estimators[keys], X_train_ = X_train, y_train_ = y_train[:,i],
                                   X_test_ = X_test, y_test_ = y_test[:,i])

with open('best_params_regression.yml', 'w') as outfile:
    yaml.dump(bestestim, outfile, default_flow_style=False)
